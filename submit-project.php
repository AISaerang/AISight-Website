...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $insight = trim($_POST['insight']) ?? null;
    $thumbnail_url = filter_var($_POST['thumbnail_url'], FILTER_VALIDATE_URL) ?? null;

    // Validasi input
    if (!$user_id) {
        $error = "Anda harus login untuk mengirim proyek.";
    } elseif (!$name || !$url || !$description) {
        $error = "Nama proyek, URL, dan deskripsi wajib diisi.";
    } elseif (!$url || strlen($url) > 2048) {
        $error = "URL tidak valid atau terlalu panjang.";
    } else {
        $stmt = $conn->prepare("INSERT INTO projects (user_id, name, url, description, insight, thumbnail_url, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'submitted', NOW(), NOW())");
        if ($stmt->execute([$user_id, $name, $url, $description, $insight, $thumbnail_url])) {
            $success = "Proyek berhasil ditambahkan!";
        } else {
            $error = "Terjadi kesalahan saat menyimpan proyek. Coba lagi.";
        }
    }
}
?>
...
<head>
  ...
  <meta name="description" content="Kirim proyek Anda untuk ditampilkan di portofolio AISight."/>
  <meta property="og:title" content="AISight — Submit Proyek"/>
  <meta property="og:description" content="Kirim proyek Anda untuk ditampilkan di portofolio AISight."/>
  <meta property="og:type" content="website"/>
  <title>AISight — Submit Proyek</title>
  <style>
    :root {
      --bg: #0B1220;
      --surface: #0F172A;
