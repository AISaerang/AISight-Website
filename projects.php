...
if ($user_id) {
    ...
    $stmt = $conn->prepare("SELECT p.id, p.name, p.url, p.description, p.insight, u.name as tutor FROM projects p JOIN users u ON p.user_id = u.id WHERE p.user_id = ? ORDER BY p.created_at DESC");
                ...
                <div class="project-list">
                    <?php foreach ($projects as $project): ?>
                        <div class="project-card">
                            ...
                            <p><strong>Tutor:</strong> <?php echo htmlspecialchars($project['tutor']); ?></p>
                            <p><?php echo htmlspecialchars(substr($project['description'], 0, 100) . (strlen($project['description']) > 100 ? '...' : '')); ?></p>
                            <a href="projects.php?id=<?php echo $project['id']; ?>">Lihat Detail</a>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
